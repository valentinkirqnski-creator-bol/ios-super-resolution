...
// --- Host helpers for the HHSR integration ---------------------------------
...
kernel void upsample_nearest_int(texture2d<int, access::read> in_texture [[texture(0)]],
    ...
    x = clamp(x, 0, int(in_texture.get_width()) - 1);
    y = clamp(y, 0, int(in_texture.get_height()) - 1);
    out_texture.write(in_texture.read(uint2(x, y)), gid);
}

// Separable binomial blur (kernel_size==2 is used between pyramid levels).
kernel void blur_mosaic_texture(texture2d<float, access::read> in_texture [[texture(0)]],
    ...
    else {bw[0]=6; bw[1]=4; bw[2]=1; kernel_size_trunc = 2;}

    float total_intensity = 0.0f;
    float total_weight = 0.0f;
    uint2 xy;
    xy[1 - direction] = gid[1 - direction];
    int const i0 = int(gid[direction]);
    ...
    out_texture.write(total_intensity / max(total_weight, 1e-6f), gid);
}

kernel void float_buf_to_texture(device const float* src [[buffer(0)]],
                                 texture2d<float, access::write> dst [[texture(0)]],
                                 constant int& width [[buffer(1)]],
                                 constant int& height [[buffer(2)]],
                                 uint2 gid [[thread_position_in_grid]]) {
    if (gid.x >= uint(width) || gid.y >= uint(height)) return;
    float v = src[gid.y * uint(width) + gid.x];
    dst.write(v, gid);
}

kernel void seed_alignment_const(texture2d<int, access::write> out_texture [[texture(0)]],
                                 constant int& dx [[buffer(0)]],
                                 constant int& dy [[buffer(1)]],
                                 uint2 gid [[thread_position_in_grid]]) {
    if (gid.x >= out_texture.get_width() || gid.y >= out_texture.get_height()) return;
    out_texture.write(int4(dx, dy, 0, 0), gid);
}

// Map HDR+ overlapping-tile alignments onto HHSR's non-overlapping tile FlowField.
// HHSR tile (tx,ty) corresponds to HDR+ tile (2*tx, 2*ty).
kernel void alignment_to_hhsr_flow(texture2d<int, access::read> alignment [[texture(0)]],
                                   device float* flow [[buffer(0)]],
                                   constant uint& flow_nx [[buffer(1)]],
                                   constant uint& flow_ny [[buffer(2)]],
                                   constant float& pixel_scale [[buffer(3)]],
                                   uint2 gid [[thread_position_in_grid]]) {
    if (gid.x >= flow_nx || gid.y >= flow_ny) return;
    uint aw = alignment.get_width();
    uint ah = alignment.get_height();
    uint ax = min(gid.x * 2u, aw > 0u ? aw - 1u : 0u);
    uint ay = min(gid.y * 2u, ah > 0u ? ah - 1u : 0u);
    int4 a = alignment.read(uint2(ax, ay));
    uint i = (gid.y * flow_nx + gid.x) * 2u;
    flow[i + 0u] = float(a.x) * pixel_scale;
    flow[i + 1u] = float(a.y) * pixel_scale;
}
