        ...
        for (int x = 0; x < out_w; ++x) {
            ...
            if (!is_ref && flow && tile_size > 0 && flow->ny > 0 && flow->nx > 0 &&
                ...
                if (bilinear_flow) {
                    ...
                    // sample_nearest rather than indexing the coarse grid: it
                    // follows the fine grid when one exists, matching the GPU
                    // twin (flow_gpu_grid swaps it in unconditionally). The
                    // mask has to score the correspondence the merge fetches.
