...
static Image compute_robustness_metal_raw_res_impl(const Image& comp_raw,
    ...
    // ONE curve shared by every guide channel, matching python-p: its
    // std_curve/diff_curve are 1-D (brightness only, no channel axis) and are
    // built from a single scalar alpha/beta. The concatenated 3-slot buffer
    // layout is kept so rob_make_mask's std_curve[ch*p.curve_n + id] indexing
    // is unchanged -- all three slots now just hold the same curve.
    ...
    id<MTLBuffer> b_chain = chain_on
        ...
        : buf(nullptr, sizeof(uint32_t));
    const bool magnitude_on = cfg.motion_magnitude_veto_enabled &&
                              flow.motion_magnitude_reject.size() == n_tiles;
    id<MTLBuffer> b_magnitude = magnitude_on
    ...
    mp.motion_magnitude_veto_enabled = magnitude_on ? 1u : 0u;
...
bool merge_comp_band_metal(const Image& comp_raw, const FlowField& flow,
    ...
    p.raw_res_robustness = cfg.robustness_raw_resolution_active() ? 1u : 0u;
