...
struct RobMaskParamsCPU {
    ...
    // Exposure-invariant geom rejection (Config::motion_geom_relative).
    uint32_t geom_relative = 1;
    float    geom_noise_floor_mult = 1.5f;
    float    geom_reject_threshold_relative = 0.04f;
    // Flat-pixel threshold; geom_reject_threshold is the textured one.
    float    geom_reject_threshold_flat = 0.0045f;
};
...
static Image compute_robustness_metal_impl(const Image& comp_raw, const RefStats& ref_stats,
    ...
    mp.geom_reject_threshold_flat = cfg.motion_geom_reject_threshold_flat;
    id<MTLBuffer> b_match_amb = amb_on
        ? buf(flow.match_ambiguous.data(), flow.match_ambiguous.size() * sizeof(uint32_t))
        : b_motion;
