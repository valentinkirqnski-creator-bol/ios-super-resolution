...
static void accumulate_comp(const Image& img, const FlowField& flow, const CovField& covs,
    ...
    parallel_rows(band_h, cfg.num_threads, [&](int local_i) {
        ...
        for (int hr_j = 0; hr_j < Ws; ++hr_j) {
            ...
            // Flow hypotheses. Normally one (sampled); in overlapped-tile merge
            // mode (Config::flow_overlap_merge) up to four -- the covering
            // tiles' OWN vectors on the tile-centre lattice, Hann-crossfaded
            // (cos^2/sin^2 per axis == the 50%-overlap raised cosine, a
            // partition of unity). Duplicates are merged so smooth regions cost
            // a single gather. See Config::flow_overlap_merge.
            f32 hx[4], hy[4], hw[4] = {1.f, 0.f, 0.f, 0.f};
            int nhyp = 1;
            if (cfg.overlap_merge_active()) {
                const f32 P = (f32)tile_size;
                ...
                const int iy0 = cl(cy0i, flow.ny), iy1 = cl(cy0i + 1, flow.ny);
                const int ix0 = cl(cx0i, flow.nx), ix1 = cl(cx0i + 1, flow.nx);
                ...
                for (int a = 0; a < 4; ++a) {
                    hx[a] = flow.dx(idx4[a][0], idx4[a][1]);
                    hy[a] = flow.dy(idx4[a][0], idx4[a][1]);
                    hw[a] = w4[a];
                }
                // Fold identical hypotheses so a smooth pixel is one gather.
                for (int a = 1; a < 4; ++a)
                    for (int b = 0; b < a; ++b)
            ...
            // Up to four hypotheses per pixel; each warps under its own tile
            // vector and its window-weighted result adds into the same num/den.
            f32 val[3] = {0, 0, 0}, acc[3] = {0, 0, 0};
            for (int h = 0; h < nhyp; ++h) {
            if (hw[h] <= 0.f) continue;
            const f32 flowx = hx[h], flowy = hy[h], wsc = hw[h];
