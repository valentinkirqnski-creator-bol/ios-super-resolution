...
static inline f32 sample_robustness_bilinear_ch(const Image& robustness, f32 y, f32 x, int ch) {
...
static inline f32 sample_robustness_bilinear(const Image& robustness, f32 y, f32 x) {
    return sample_robustness_bilinear_ch(robustness, y, x, 0);
}
...
static void accumulate_comp(const Image& img, const FlowField& flow, const CovField& covs,
    ...
    parallel_rows(band_h, cfg.num_threads, [&](int local_i) {
        ...
        for (int hr_j = 0; hr_j < Ws; ++hr_j) {
            ...
            // ISA (robustness.c == 3) carries PER-CHANNEL R: each colour's own
            // robustness gates its own accumulation. Scalar masks (c == 1)
            // apply one value to every channel, as before.
            const bool rob_per_ch = (robustness.c == 3);
            f32 local_r_ch[3];
            if (rob_per_ch) {
                for (int rc = 0; rc < 3; ++rc)
                    local_r_ch[rc] = sample_robustness_bilinear_ch(robustness, rob_y, rob_x, rc);
            } else {
                const f32 s0 = sample_robustness_bilinear(robustness, rob_y, rob_x);
            ...
            for (int di = -1; di <= 1; ++di) {
                for (int dj = -1; dj <= 1; ++dj) {
                    ...
                    val[channel] += w * local_r_ch[channel] * c;
                    acc[channel] += w * local_r_ch[channel];
                }
            }
