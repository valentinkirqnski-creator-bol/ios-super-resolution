...
struct CameraView: View {
    ...
    private var fineAlignmentSection: some View {
        ...
        Text("""
             Bilinear flow sampling is right where motion is smooth but wrong at object              boundaries: flow is only piecewise smooth, so blending two motions produces              flow that belongs to neither side -- exactly where robustness then rejects              and detail is lost. This builds a half-tile-pitch refinement after alignment:              cells whose four surrounding tile vectors agree keep the bilinear blend,              while cells at a disagreement over 1 raw px are MEASURED on an overlapping              full-tile window (HDR+'s tile 16 / offset 8 scheme): best neighbour vector              as seed, 3x3 refinement, quadratic sub-pixel fit; the measurement replaces              the seed only when it actually finds a different motion. Mask and merge              consume the same refined field, so they stay in lockstep. Requires Smooth              Tile Flow.
             """)
            .font(.caption2).foregroundColor(.secondary)
