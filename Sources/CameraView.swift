...
struct CameraView: View {
    ...
    private var fineAlignmentSection: some View {
        ...
        Text("The HDR+ scheme, as the reference author suggests trying first: alignment tiles of Ts=16 at stride 8 (50% overlap), each half-pitch position measured on its own full-tile window -- flow is NOT interpolated -- and the merge blends each output pixel's up to four covering-tile results with the raised-cosine window (a Hann crossfade at 50% overlap, summing to one). Where the tiles agree the hypotheses deduplicate and the merge costs exactly what it does today; where they disagree the results crossfade instead of the flow blending. Decimate mode only. Works with Smooth Tile Flow either on or off: the overlap merge reads each covering tile's vector nearest and never interpolates the flow itself. With Smooth Tile Flow OFF the field is piecewise-constant with no interpolation anywhere -- but note it is still constant at the COARSE tile pitch, because a cell whose neighbours agree just takes its containing tile's vector; it does NOT reduce the rotation staircase on its own. Turn on Measure Every Overlap Tile for that. The robustness mask follows the same grid in every case. Experimental; off by default.")
            .font(.footnote)
            .foregroundColor(.secondary)
        Toggle("Measure Every Overlap Tile", isOn: $cam.tuningParams.flow_overlap_measure_all)
        Text("Aligns every half-pitch overlap cell on its own full-tile window, which is what HDR+ actually does -- the paper overlaps tiles by half for BOTH alignment and merging, and accepts 4x the alignment operations for it. That is why HDR+ needs no interpolated flow: nearest is exact when every cell has a real measurement. Without this, the overlap merge only measures cells whose four neighbouring tile vectors disagree and derives the rest from the coarse grid, so the field stays at the coarse tile pitch and rotation still steps by theta*16 rather than theta*8. Costs what the paper says: 4x the lattice alignment work, and measured vectors are never bit-equal so the merge stops collapsing agreeing hypotheses and pays up to 4 gathers per pixel -- it was gated off for exactly this reason. Requires Overlapped Tile Merge. Experimental; off by default.")
            .font(.footnote)
            .foregroundColor(.secondary)
