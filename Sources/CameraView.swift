...
struct CameraView: View {
    ...
    private var fineAlignmentSection: some View {
        ...
        Text("""
             ...
             This accumulates R-G and B-G instead of R and B, then adds the sharp green back.              R-G is smooth across an edge even when R and G separately are not, so all the              edge structure comes from green, which always has samples on its own row.
        ...
        Text("""
             ...
             """)
            .font(.caption2).foregroundColor(.secondary)
        Toggle("Robustness in sRGB Space", isOn: $cam.tuningParams.robustness_color_space)
        Text("""
             Experimental (the reference author's cross-camera direction): the robustness              statistics (d and sigma) are computed in colour-transformed linear sRGB instead of              the camera-native space, with the noise model transformed to match. One tuning              then works across cameras, and the matrix's off-diagonals amplify chroma-coherent              differences -- the colour-fringe signature -- so chroma-visible misalignment residue              is rejected slightly more (measured: worst-crop chroma max 116 -> 108, never worse).              It cannot catch wrong-period texture locks (their d stays ~0 in any colour space).              While on, the mask computes on the CPU golden path (no Metal twin yet) -- expect a              slower analyze stage. The formulas (Eq. 5-9, s1/s2/t, erosion) are unchanged; only              the space they measure in.
             """)
            .font(.caption2).foregroundColor(.secondary)
        Toggle("ISA", isOn: $cam.tuningParams.isa_mode)
        Text("""
             ImageStackAlignator, algorithmically identical to ImageStackAlignator-master              (the vendored 1:1 port, verified stage by stage against the original C#/CUDA):              full-resolution rotation-scan pre-alignment, all-pairs patch tracking with the              global shift-consistency solve, dense per-pixel Lucas-Kanade flow, ISA's own              robustness mask and kernel-regression merge (kDetail 0.25, kDenoise 3, kStretch 4,              kShrink 2), and ISA's colour rendering baked into the output DNG at input              resolution. Currently the exact CPU implementation (all cores) -- expect several              minutes per burst, dominated by the rotation scan; Metal ports of the stages land              incrementally, each verified against this reference. Overrides HDR+ Merge and every              super-resolution setting while on.
             """)
            .font(.caption2).foregroundColor(.secondary)
        Toggle("HDR+ Merge (denoise, no super-res)", isOn: $cam.tuningParams.hdrplus_mode)
        Text("""
             Replaces the whole super-resolution pipeline with the HDR+ burst align +              merge (Google's, via the hdr-plus-master Halide implementation, ported to this              core with a Metal path and a CPU fallback): 3-level L1 pyramid alignment of              32x32 mosaic tiles, then a temporal weighted average per tile (weight inverse              to the tile's L1 distance from the reference) blended with a raised-cosine              window. A denoiser, not an upscaler -- the output DNG is at sensor resolution              (demosaicked bilinearly), and every super-resolution setting (kernels,              robustness, flow, scale) is bypassed while this is on. Ghosting protection is              HDR+'s own tile weighting, not the Wronski robustness mask.
             """)
            .font(.caption2).foregroundColor(.secondary)
        ...
        Text("""
             ...
             It re-estimates from image data, so it has its own floor (~0.055 px measured).              It wins clearly once per-tile estimation noise exceeds that, and cannot help a              field already more accurate than its floor. CPU-only for now: expect it to be              slow until there is a Metal kernel for it.
             """)
            .font(.caption2).foregroundColor(.secondary)
