...
struct CameraView: View {
    ...
    private var fineAlignmentSection: some View {
        ...
        Text("HDR+'s merge scheme, as the reference describes it: alignment tiles of Ts at stride Ts/2 (overlapped by half), each half-pitch tile ALIGNED in its own right, and the merge blending each output pixel's up to four covering-tile results with the raised-cosine window -- a Hann crossfade at 50% overlap, which sums to exactly 1. The flow is never interpolated: each covering tile contributes its own measured vector, read nearest, and continuity between tiles comes from the window rather than from smoothing the field. That is how HDR+ avoids the blocky tile seams without blending flow across motion boundaries. Works with Smooth Tile Flow on or off -- it does not need it. Decimate mode only. Costs what the paper says it costs: 4x the alignment operations over the lattice. Experimental; off by default.")
            .font(.footnote)
            .foregroundColor(.secondary)
        Toggle("Measure Every Overlap Tile", isOn: $cam.tuningParams.flow_overlap_measure_all)
        Text("On (default) each half-pitch cell is aligned on its own full-tile window, which is what makes the above actually HDR+. Off, only cells whose four neighbouring tile vectors disagree are measured and the rest inherit the containing coarse tile's vector, so the field stays at the coarse tile pitch and the half-overlap buys only the window. Requires Overlapped Tile Merge.")
            .font(.footnote)
            .foregroundColor(.secondary)
