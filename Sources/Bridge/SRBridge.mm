...
//
// Deliberately does NOT take the preset-LUT branch that
// render_linear_dng_pixel offers: that table was fitted straight from linear
// DNG RGB to a finished JPEG, so it has a tone curve and gamma baked in.
// Feeding its output to the HDR+ pipeline would tone-map twice.
...
static void ApplyTuningParams(NSDictionary<NSString *, NSNumber *> *tuning, Config& cfg) {
    ...
    if (tuning[@"flow_overlap_measure_all"])
        cfg.flow_overlap_measure_all = tuning[@"flow_overlap_measure_all"].boolValue;
...
+ (BOOL)exportJPEGFromLinearDNG:(NSString *)dngPath
    ...
    // HDR+ finishing (IPOL Section 5.2), replacing the old ISP. It is a
    // whole-image pipeline -- the exposure fusion and the three blur scales
    // are neighbourhood operations -- so unlike isp_render it cannot be folded
    // into the per-pixel loop below.
...
+ (BOOL)embedJPEGPreviewInDNG:(NSString *)dngPath maxSide:(NSInteger)maxSide {
    ...
    // Finished at full resolution even though the preview is downscaled, so
    // the thumbnail and the exported JPEG get the same auto gain and the same
    // tonal result and cannot disagree about how the shot looks. HDR+
    // finishing is a whole-image pipeline, so it runs once up front rather
    // than per sampled pixel.
    const bool use_hdrplus = g_isp.enabled;
    hhsr::Image fin;
